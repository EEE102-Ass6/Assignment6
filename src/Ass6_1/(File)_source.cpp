...
bool Driver_File::File_To_Hashtable() {
	...
	ifstream acc_hash;
	//read only
	acc_hash.open(file_name_d, ios::in | ios::binary);
	if (acc_hash.fail()) {
		cout << "Error! Cannot open the file..." << endl; Sleep(2000);
		acc_hash.close();
		return false;
//		std::exit(0);
		...
		for (int i = 0; !acc_hash.eof(); i++) {
			if (!acc_hash.read((char*)user_temp, sizeof(user_info)))
...
void Driver_File::Modify(Driver &obj_origin, Driver &obj_new) {
	...
	//amend in hashtable:
	this->hash_table_D.Hash_Amend(obj_origin, obj_new);

	/*--------------------amend in file:--------------------*/

	FILE *ptr_account;
	if ((ptr_account = fopen(file_name_d, "rb+")) == NULL) {	//'r+': read and amend file information
		...
		fseek(ptr_account, (obj_origin.Get_info()->position_in_file - 1) * sizeof(user_info), SEEK_SET);
		fwrite(obj_new.Get_info(), sizeof(user_info), 1, ptr_account);
	}
	fclose(ptr_account);
...
void Driver_File::Delete(Driver &obj_origin) {
	...
	if (ptr_driver == NULL || ptr_temp == NULL) {
		...
		while (fread(&temp, sizeof(user_info), 1, ptr_driver)) {
			if (strcmp(temp.Driver_link.driver_Num, obj_origin.Get_info()->Driver_link.driver_Num) != 0 &&
				strcmp(temp.Driver_Tel, obj_origin.Get_info()->Driver_Tel) != 0)
	...
	rename("temp.bat", file_name_d);	//change name into origin file
...
bool Vehicle_File::File_To_Hashtable() {
	...
	ifstream acc_hash;
	//read only
	acc_hash.open(file_name_v, ios::in | ios::binary);
	if (acc_hash.fail()) {
		cout << "Error! Cannot open the file..." << endl; Sleep(2000);
		acc_hash.close();
		return false;
//		std::exit(0);
		...
		cout << "Hashtable is creating..." << endl;
		for (int i = 0; !acc_hash.eof(); i++) {
			if (!acc_hash.read((char*)car_temp, sizeof(car_info)))
				break;	//this time, read failed...
			else {
				/*----------Expanding----------*/
				//create an object:
...
void Vehicle_File::addNew(car_info *struct_new) {
	...
	cout << struct_new->position_in_file << endl;
	std::system("PAUSE");
...
void Vehicle_File::Modify(Vehicle &obj_origin, Vehicle &obj_new) {
	...
	if ((ptr_account = fopen(file_name_v, "rb+")) == NULL) {	//'r+': read and amend file information
		...
		fseek(ptr_account, (obj_origin.Get_info()->position_in_file - 1) * sizeof(car_info), SEEK_SET);
...
void Vehicle_File::Delete(Vehicle &obj_origin) {
	...
	rename("temp.bat", file_name_v);	//change name into origin file
